...
class ViewController: UIViewController {
  ...
  private lazy var subview1: UITextView = {
    let subview1 = UITextView()
...
extension ViewController: UIScrollViewDelegate {
  func scrollViewDidScroll(_ scrollView: UIScrollView) {
    ...
    let heightOfSubview1 = subview1.bounds.height
    let heightOfSubview2 = subview2.bounds.height
    
    if lastContentOffsetY < contentOffsetY { targetOffset = Int(heightOfSubview1 + heightOfSubview2 / 2  - (scrollHeight / 2)) }
    else { targetOffset = Int((heightOfSubview1 / 2) - (scrollHeight / 2)) }
    
    lastContentOffsetY = contentOffsetY
  }
  
  func scrollViewWillEndDragging(_ scrollView: UIScrollView, withVelocity velocity: CGPoint, targetContentOffset: UnsafeMutablePointer<CGPoint>) {
    targetContentOffset.pointee = CGPoint(x: 0, y: self.targetOffset)
  }
}
